...
function handleKeyPress(key) {
    ...
    if (key === 'ENTER') {
        if (isGuessComplete()) {
            ...
            showMessage("Guess is not complete!", 'error', 2000);
        }
    }
...
function submitGuess() {
    ...
    let checkLetterResults = []

    if (isGuessComplete()) {
        if (!WordleWords.isValidWord(currentGuess)) {
            showMessage("Word is not valid!", 'error', 2000);
            shakeRow(currentRow);
            return;
        } 
        else {
            for (let i = 0; i < currentGuess.length; i++) {
                checkLetterResults.push(checkLetter(currentGuess[i], i, currentWord));
            }
            for (let i = 0; i < checkLetterResults.length; i++) {
                const tile = getTile(currentRow, i);
                setTileState(tile, checkLetterResults[i]);
            }
            updateKeyboardColors(currentGuess, checkLetterResults);
            if (currentGuess === currentWord) {
                gameWon = true;
                gameOver = true;
            }
            updateGameState(currentGuess === currentWord);
...
function checkLetter(guessLetter, position, targetWord) {
    ...
    const uppercaseGuessLetter = guessLetter.toUpperCase();
    const uppercaseTargetWord = targetWord.toUpperCase();
    const uppercaseCurrentGuess = currentGuess.toUpperCase();
    
    // TODO: Check if letter is in correct position
    // HINT: Compare targetWord[position] with guessLetter
    if (uppercaseTargetWord[position] === uppercaseGuessLetter) {
        return 'correct';
    }
    
    const lettersInTarget = uppercaseTargetWord.split(uppercaseGuessLetter).length - 1;
    console.log(lettersInTarget);
    if (lettersInTarget === 0) {
        return 'absent';
    }

    let numLettersUsed = 0;
    for (let i = 0; i < uppercaseCurrentGuess.length; i++) {
        if (uppercaseCurrentGuess[i] === uppercaseGuessLetter && uppercaseTargetWord[i] === uppercaseGuessLetter) {
            numLettersUsed++;
        }
    }

    for (let i = 0; i < position; i++) {
        if (uppercaseCurrentGuess[i] === uppercaseGuessLetter && 
             uppercaseTargetWord[i] !== uppercaseGuessLetter && uppercaseTargetWord.includes(uppercaseGuessLetter)
        ) {
            numLettersUsed++;
        }
    }

    if (numLettersUsed < lettersInTarget) {
        return 'present';
    }
    else {
        return 'absent';
    ...
    // if (uppercaseTargetWord.includes(uppercaseGuessLetter)) {
    //     return 'present';
    // }
...
function showEndGameModal(won, targetWord) {
    ...
    if (won) {
        message = `You guessed it in ${guessCount} guesses!`;
    } else {
    ...
    showModal(won, targetWord, guessCount);
